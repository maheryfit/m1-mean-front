...
export class MecanicienService{
  ...
  getDetailsRdv(idrdv:string){
    const url=`${environment.API_URL}/mecanicien/details-rdv/${idrdv}`;
    const xhr=new XMLHttpRequest();
  ...
  ajouterDiagnostic(idrdv:string,diagnostic:any){
    const url=`${environment.API_URL}/mecanicien/ajouter-diagnostic/${idrdv}`;
    const xhr=new XMLHttpRequest();
    const promise=new Promise<void>(function (resolve,reject){
      ...
      xhr.setRequestHeader("Content-type","application/json;charset=utf-8");
      xhr.withCredentials=true;
      xhr.send(JSON.stringify(diagnostic));
    });
    return promise;
  }
  prendreChargeRdv(idrdv:string){
    const url=`${environment.API_URL}/mecanicien/prendre-charge-rdv/${idrdv}`;
    const xhr=new XMLHttpRequest();
    const promise=new Promise<ClasseMecanicien>(function (resolve,reject){
      xhr.onreadystatechange=function(){
        if(this.readyState===4){
          switch(this.status){
            case 200:
              const response=JSON.parse(this.response);
              const mecanicien=new ClasseMecanicien();
              mecanicien.init(response);
              resolve(mecanicien);
              break;
            case 500:
              reject(JSON.parse(this.response).message);
