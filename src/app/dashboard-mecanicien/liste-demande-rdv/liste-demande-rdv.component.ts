...
export class ListeDemandeRdvMecanicienComponent {
  count=signal(0);
  ...
  demandes=signal<DemandeRdvMecanicien[]>([]);
  constructor(){
    effect(()=>{
      this.demandeService.getAllForMecanicien(this.currentIndex(), this.pageLimit).subscribe({
        next: (data)=>{
